...
// modified by Shin Adachi <shn@glucose.jp>
//  * Add import / export
...
export function fts_fuzzy_match_async(matchFn, pattern, dataSet, onComplete) {
  let ITEMS_PER_CHECK = 1000         // performance.now can be very slow depending on platform
  ...
  function step() {
    ...
    let stopTime = performance.now() + max_ms_per_frame
    ...
    for (; dataIndex < dataSet.length; ++dataIndex) {
      ...
      let str = dataSet[dataIndex]
      let result = matchFn(pattern, str)
